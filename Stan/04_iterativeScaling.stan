//This model is vectorized, and uses the fast Fourier transform in the convolution.
//This model is compatible with the iterative wrapper.
//This model estimates a scaling coefficient, beta, to modify the rainwater input R.
data {
 ...
 //the prior estimates of the parameters, passed in from the previous iteration.
 real<lower=0> eta_mu;
...
parameters {  
  //shape parameters for the impulse response function.
  real<lower=0> lambda;     
  real<lower=0> eta;
  //the standard deviation of measurements, i.e. measurement error.
  real<lower=0> sigma_obs; 
  //scaling coefficient for rainwater input.
  real<lower=0> beta;
}
...
transformed parameters{
  ...
  complex_vector[N] inv_fft_product = inv_fft(elt_multiply(fft(IRF),fft(R*beta)));
